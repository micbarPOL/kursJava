...
public class PrzelicznikWalutOkno2 {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					PrzelicznikWalutOkno2 window = new PrzelicznikWalutOkno2();
	...
	protected void przelicz() {
		try {
			...
			if(rdbtnWalutaNaZlote.isSelected()) {
				wynik = waluta.walutaNaZloteZaokragalnie(kwota);
			}
			if(rdbtnZloteNaWalute.isSelected()) {
				wynik = waluta.zloteNaWalute(kwota);
			}
	...
	private void pobierzKursyDlaDaty() {
		SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
			protected Void doInBackground() throws Exception {
				...
				Tabela nowaTabela = ObslugaNBP.pobierzArchiwalneKursy(data);
				if(nowaTabela != null) {
					tabela = nowaTabela;
				} else {
					// TODO napisać gdzieś że był błąd
				}
				return null;
			}
